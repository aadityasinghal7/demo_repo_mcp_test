...
def test_groupweightedaverage_error_handling():
    ...
    # weights column missing
    with pytest.raises(KeyError):
        groupweightedaverage(df, groupby="group", value="value", weights="missing_weight")

    # non-numeric 'value' column
    df_non_numeric_value = df.copy()
    df_non_numeric_value["value"] = ["x", "y", "z"]
    with pytest.raises(TypeError):
        groupweightedaverage(df_non_numeric_value, groupby="group", value="value", weights="weight")

    # non-numeric 'weight' column
    df_non_numeric_weight = df.copy()
    df_non_numeric_weight["weight"] = ["x", "y", "z"]
    with pytest.raises(TypeError):
        groupweightedaverage(df_non_numeric_weight, groupby="group", value="value", weights="weight")